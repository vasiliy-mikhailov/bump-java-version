#!/usr/bin/env python3
"""Unified 3-agent sweep (inner loop). One image (bump-allagents-sweep) + one driver; AGENT is the
only variable. Paths from sibling _paths.
Usage: OC_KEY=... python3 agent_sweep.py <opencode|kilo|openhands> [N=40] [K=3] [OFF=0]"""
...
SKILL = str(P.SKILL); CFG = str(P.CFG); DRIVE = str(P.DRIVE); OHRUN = str(P.OHRUN)
OUT = str(P.OUT / ("sweep3_" + AGENT)) + os.environ.get("OUT_SUFFIX", "")
M2 = str(P.M2); SETTINGS = str(P.SETTINGS)
IMAGE = "bump-allagents-sweep:latest"
OC_KEY = os.environ["OC_KEY"]
ds = json.load(open(os.environ.get("DATASET_FILE", str(P.DATASET))))
NEXT = {8: 11, 11: 17, 17: 21, 21: 25}
byhop = defaultdict(list)
for e in ds: byhop[f"{e['jv_from']}->{NEXT[e['jv_from']]}"].append(e)
hops = sorted(byhop); per = N // len(hops)
examples = []
for h in hops: examples += byhop[h][OFF:OFF + per]
examples = examples[:N]
...
def run_one(e):
    ...
    slug = repo.replace("/", "_") + "_" + sha[:8]; cname = "a3_" + uuid.uuid4().hex[:10]
