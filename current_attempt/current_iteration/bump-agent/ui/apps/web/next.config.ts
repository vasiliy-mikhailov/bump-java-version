 ...
 * single-process — one JVM, no node beside it — in an image that already runs arbitrary code from
 * strangers' repositories. The cost is no RSC and no server-side theme cookie; the theme is handled
 * by a blocking script instead (see app/layout.tsx), which avoids the flash the same way.
 */
...
export default {
  ...
  // which distinguishes "absent" from "present and undefined", and Next means the first.
