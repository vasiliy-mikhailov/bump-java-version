...
if [ ! -f "$O/verdict.txt" ]; then
  ...
  docker run --rm -v "$RUN/hoptest:$RUN/hoptest" -v "$CI/rung2/r2score_one.py:/s.py:ro" python:3-slim python3 /s.py "$O" "$REPO" >/dev/null 2>&1
fi
docker run --rm -v "$RUN/ws:/w" alpine rm -rf "/w/$SLUG" 2>/dev/null || true
echo "DONE $SLUG"
