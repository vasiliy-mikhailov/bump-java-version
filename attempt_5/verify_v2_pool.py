"""Verify v2 lineage candidates that reach J21. Higher concurrency than v1
verifier per fitness #8 (CPU underutilization noted).

Reads attempt_5/lineage_candidates_v2.json, runs each candidate's commits
through docker baseline-build under matching JDK.
"""
...
def verify_one_commit(repo_url, sha, jv):
    tmp = tempfile.mkdtemp(prefix="lv2-")
    try:
        ...
        cname = f"lv2-{os.getpid()}-{threading.get_ident()}"
...
def main():
    cands = json.load(open(f"{HERE}/attempt_5/lineage_candidates_v2.json"))
    ...
    out_all = f"{HERE}/attempt_5/lineage_verified_v2.json"
    json.dump(verified, open(out_all, "w"), indent=2)
    full = [e for e in verified if e["all_pass"]]
    out = f"{HERE}/attempt_5/lineage_dataset_v2.json"
