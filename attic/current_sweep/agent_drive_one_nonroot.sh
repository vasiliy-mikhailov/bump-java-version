...
cd $HOME; rm -rf work; mkdir work; cd work
...
case "$AGENT" in
  opencode)
    mkdir -p "$HOME/.config/opencode"; cp /cfg/opencode.json "$HOME/.config/opencode/opencode.json"
    timeout 1800 opencode run -m qwen/$QWEN_MODEL "$PROMPT" > "$OUT/agent.log" 2>&1; echo "agent rc=$?" >> "$OUT/agent.log" ;;
  kilo|kilocode)
    mkdir -p "$HOME/.config/kilo"; cp /cfg/kilo.json "$HOME/.config/kilo/opencode.json"
