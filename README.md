...
| 7 | per-repo iterative search over a sequenced default chain (`lombok_bump → java8→11 → plugins17 → build17 → java17_transforms → plugins21 → build21 → java21_transforms`) + Qwen-proposed per-repo mutations + rewrite-maven-plugin bumped 6.12.0 → 6.40.0 (fixed `RawGradleModule.parse` Nexus-404-HTML chokepoint) | 395 J21-target stages | **91 %** | **+24 pp over iter-0 baseline (67 → 91)** |
