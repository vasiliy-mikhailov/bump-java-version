...
4. **Fitness (recipe):** find a per-stage OpenRewrite recipe composition that converts each repo in `java21-migration-dataset.json` toward the same form the repo's human maintainers committed at each Java level.
    ...
    - **Search:** for each stage, draw candidate compositions from the OpenRewrite recipe catalog, from community-attested migration guidance, and from a per-repo comparison between the candidate's output and the human's actual commit at the stage's output level on the same repo — the diff against the human is the ground-truth feedback that drives mutation.
    - **Reward:** at each stage, fraction of the corpus that builds on the stage's JDK after the recipe runs, jointly with how close the recipe's output gets to the human's commit at that level (matching files first, line-set delta second); regressing a previously-building repo costs more than failing to improve a non-building one.
    - **Repeat:** ralph loop per stage; when a stage's coarse metric plateaus, drop into item 7.
