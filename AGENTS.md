...
7. **Fitness (per-failing-repo refinement):** a fine-grained ralph loop nested under fitness 4 — invoked when coarse whole-recipe mutations stop moving the corpus build-success metric.
   - **Constraints:** declarative configuration deltas only — no bespoke Java AST recipes. Recipe order matters. A primitive that fires correctly but doesn't flip its target stays unless it regresses the corpus.
   - **Search:** for each still-failing repo, root-cause the first failing import or plugin, then ask any reachable source for the known fix — including vLLM Qwen 3.6 27B FP8 and Claude.
   - **Reward:** concrete `build_post 0 → 1` flips net of corpus regressions. Use Qwen as diff judge and Claude as candidate-primitive judge to filter before paying smoke wall-time.
   - **Repeat:** pick the cheapest cluster, diagnose, compose, smoke on a few targeted repos, commit. Terminate when every remaining cluster needs bespoke engineering.
