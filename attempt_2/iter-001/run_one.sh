...
# Outputs to attempt_2/iter-001/results/<REPO_ID>/{metrics.json, diff.patch, run.log, per_recipe.csv}
set -uo pipefail
HERE=/home/vmihaylov/java_8_11_17_to_java_21
RECIPE_YML="$HERE/attempt_2/iter-001/_recipe/rewrite.yml"
OUT_DIR="$HERE/attempt_2/iter-001/results/$REPO_ID"
mkdir -p "$OUT_DIR"
docker run --rm \
  -e REPO_URL="$REPO_URL" \
  -e REPO_SHA="$REPO_SHA" \
  ...
  -e RECIPE_NAME=com.fitness.iter1.MigrateToJava21 \
