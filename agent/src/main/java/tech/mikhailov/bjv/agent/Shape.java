 ...
 * nesting, how often each one ran and which list each one worked to, beside a {@link Bump} that did
 * all five for real. Two copies of one fact drift, and every part of that pair drifted at least
 * once: the page advertised a {@code prepare} stage for hours after all three of its agents were
 * deleted, a label said a phase ran once for the repository days after it became per module, and a
 * strip drew a loop between two stages that had never joined. Nothing failed and no test went red.
 * A reader was simply told something untrue by the page whose whole job is to say what a bump is
 * doing.
 *
 * <p>So there is no declaration left. {@link Flow} builds the bump as one tree of named nodes and
 ...
 * speaks inside a node, how often the node runs, and which half of the bill of materials it works
 * to. An agent is called from inside a body, and a condition is a {@code BooleanSupplier} with no
...
public final class Shape {
     ...
     * deterministic step rather than an agent it is marked as one: the modules stage and the repair
     ...
     * Nodes that name no step at all are scope rather than stage: {@code module} is the walk's own
     * body, and what it says is already said by the {@code per module} on the stage above it.
     *
     ...
     * under it and everything below it orphaned. {@code module-gate} names a step and prompts
     * nobody, so module-repair reports the nearest ancestor that speaks, which is modules. The tree
     * keeps the more specific truth and {@code Flow.shape} prints it: repair runs inside the gate's
     * own turn rather than beside it.
     */
    static List<Stage> of(Agents.Agent root) {
        List<Stage> out = new ArrayList<>();
        walk(root, "", out);
        return out;
    }

    private static void walk(Agents.Agent node, String within, List<Stage> out) {
        String under = within;
