...
public record Hop(int from, int to) {
    ...
    /**
     * THIS HOP AS A KEY, for the parts of the program that only file things under it.
     *
     * <p>A prompt override, a record writer and a fingerprint all need to tell one variant of the
     * pipeline from another, and none of them needs to know that the variant is a pair of Java
     * version numbers. They take this string, which is the same one that has always been the
     * directory name under the prompt store, so every override already on disk still loads.
     */
    public String key() {
        return from + "-" + to;
    }
