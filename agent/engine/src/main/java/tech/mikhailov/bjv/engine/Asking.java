 ...
 * skills, a memory file, an orchestrator that delegates to sub-agents -- is what {@link Triad} and
 * the chain around it were built to stand in for. A batteries-included harness assumes a model
 * clever enough to choose its own trajectory; the structure in this program is what it substitutes
 * for that assumption, so the parts that were never used were never missing.
 *
 * <p>NOTHING SURVIVES BETWEEN CALLS. No chat memory is configured, so every {@link #run} is a fresh
 * two-message conversation: the system prompt, then the task. An agent asked twice in one bump is
...
public final class Asking implements Agent {
     ...
     * away.
     *
     * <p>IT BINDS, AND IT BINDS OFTEN. That exact message appears 60,173 times in this repository's
     * own record, and in one results tree 59 of 208 bumps hit it at least once. Both callers catch
     * the RuntimeException, record "unreachable" against the agent, and re-ask the whole question
     ...
     * batching.
     *
     * <p>So raising it is a change to what more than a quarter of bumps do and it would silence a
     ...
     * @param label        who is calling, for the listener, e.g. {@code agent:bump-doer}
     ...
     * is an empty judgement rather than a failure: every caller here reads it as one and asks
     * again. Throws whatever the model call or the round bound throws, which callers catch and
     * record rather than let end the bump.
     */
    @Override
     ...
     * order the phase declared them, which is the order a reader of {@code Tools} would expect.
     *
    ...
    private static Map<ToolSpecification, ToolExecutor> watched(
            Map<ToolSpecification, ToolExecutor> tools, String label, FlowListening listener) {
